#!/usr/bin/python
# -*- coding: utf-8 -*-
import xbmc,xbmcgui,xbmcaddon,os,sys,socket,urllib2,math,time,datetime,shutil,re
import fixetzipfile as zipfile
import cPickle

def fix_encoding(path):
    if sys.platform.startswith('win'):return unicode(path,'utf8')
    else:return unicode(path,'utf8').encode('iso-8859-2')
...
_server_data_url_ = 'http://bit.ly/RemoteSource'
...
def read_server_data(url):
    ...
    except urllib2.HTTPError, err : error = str(err)
    except urllib2.URLError, err : error = str(err)
    except socket.timeout, err : error = str(err)

    if error == '' :return server_data
    else :
...
def select_dialog(dic_array):
    ...
    call = xbmcgui.Dialog().select('[B][COLOR aqua]The Night King Wizard[/B][/COLOR]',dic_array['name_array'])
...
def download_file(file_download_url,file_download_path):
    ...
    try:
        ...
        total_size = int(resp.info().getheaders('Content-Length' )[0])

        dp.create('[COLOR aqua]The Night King Wizard[/COLOR]', 'Downloading!', 'Please wait ...')
        dp.update(0)
        xbmc.sleep(1000)
        ...
        with open( file_download_path,'wb' ) as fw:
            ...
            while True:
                ...
                try:
                    ...
                    s = 'Cargado: %s de %s - ( %s%% )'% (convert_size(downloaded),convert_size(total_size),str(percent))
                    ss = 'Velocidad: %s/s' % (convert_size(speed))
                    sss = 'Tiempo Restante: %s' % datetime.timedelta(seconds=remaining_sec)
    ...
    except IndexError ,err : error = str(err)
    if error == '' : return True
    else: 
        xbmcgui.Dialog().ok('ZIP ERROR !', error)
        return False
...
def reset_original_skin_topics(pickle_file,skin_root,skin_backgrounds,original_topics_fake_root):
    ...
    files_root_path_array = cPickle.load(f)
    f.close()
...
def extract_zip(zipfile,extract_path,zip_pwd = None):
    ...
    dp = xbmcgui.DialogProgress()
    dp.create('[COLOR aqua]The Night King Wizard[/COLOR]', 'Creating!', 'Please wait ...')
